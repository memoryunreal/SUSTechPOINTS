...
class Root(object):
    ...
    def save(self, scene, frame):
      ...
      next_frame = "%08d" % (int(frame) + 1)
      next_frame_json = "./data/" + scene + "/label/" + next_frame + ".json"
      if not os.path.exists(next_frame_json):
        with open("./data/" + scene + "/label/" + next_frame + ".json", 'w') as f:
          f.write(rawbody)
      return "ok"
    @cherrypy.expose
